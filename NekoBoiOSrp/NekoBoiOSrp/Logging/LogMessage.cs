...
namespace NekoBoiOSrp
{
  ...
  public struct LogMessage
  {
    ...
    /// </summary>
    /// <returns>
    ///     An <see cref="MekoBoiOSrp.LogMessage.Exception" /> object associated with an incident; otherwise <c>null</c>.
    /// </returns>
    ...
    public string ToString(StringBuilder builder = null, bool fullException = true, bool prependTimestamp = true, DateTimeKind timestampKind = DateTimeKind.Local, int? padSource = 11)
    {
      ...
      if (prependTimestamp)
      {
        DateTime now = timestampKind == DateTimeKind.Utc ? DateTime.UtcNow : DateTime.Now;
        if (now.Hour < 10)
        {
          builder.Append('0');
        }
